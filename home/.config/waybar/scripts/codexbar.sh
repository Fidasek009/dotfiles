...
echo "$merged" | jq -c \
    ...
    def usage_color(p):
        if p >= 90 then "#f38ba8"
        elif p >= 70 then "#fab387"
        else "#a6e3a1"
        end;

    def progress_bar(p):
        ((p / 10) | floor) as $filled
        | (10 - $filled) as $empty
        | "<span foreground=\"\(usage_color(p))\">\("█" * $filled)</span><span foreground=\"#585b70\">\("█" * $empty)</span>";

    def reset_countdown(w):
        if w == null or w.resetsAt == null then "—"
        else
            (try (w.resetsAt | fromdateiso8601) catch null) as $reset
            | (try ($now | fromdateiso8601) catch now) as $current
            | if $reset == null then "—"
              else
                  (($reset - $current) | if . < 0 then 0 else . end | floor) as $seconds
                  | if $seconds < 3600 then "\(($seconds / 60) | ceil)m"
                    elif $seconds < 86400 then "\(($seconds / 3600) | ceil)h"
                    else "\(($seconds / 86400) | ceil)d" end
              end
        end;

    def window_text(w):
        pct_or_null(w) as $pct
        | if $pct == null then null
          else "<span foreground=\"\(usage_color($pct))\">\(reset_countdown(w))</span> \(progress_bar($pct)) <span foreground=\"\(usage_color($pct))\">\($pct)%</span>" end;

    # When the user has pinned a provider for the bar text, surface session
    # and weekly as compact progress bars. Otherwise emit the global max%.
    def bar_text(entry):
        if entry == null or entry.error then "🤖 ⚠"
        else
            [window_text(entry.usage.primary),
             window_text(entry.usage.secondary)]
            | map(select(. != null))
            | if length == 0 then "  —"
